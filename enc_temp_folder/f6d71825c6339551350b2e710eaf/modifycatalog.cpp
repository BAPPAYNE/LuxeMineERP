...
void ModifyCatalog::loadDesignForEdit(const QString& designNo)
{
    ...
    auto populateTable = [&](QTableWidget* table, const QString& jsonStr, const QString& type) {
        ...
        // Remove any existing cell widgets (safe cleanup) before changing rows.
        ...
        // Ensure table has expected columns to avoid setCellWidget/setItem crashes
        if (type == "gold") {
        ...
        for (const QJsonValue& val : arr) {
            ...
            if (type == "gold") {
                ...
                if (!shapeVal.isEmpty()) {
                    // ensure value exists in combo; if not, add it so setCurrentText works
                ...
                if (sizes.isEmpty()) {
                    // if DB returned empty, still allow setting value
                ...
                // safe because we ensured columnCount >= 3 earlier
